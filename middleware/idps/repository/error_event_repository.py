"""
Repository SQLAlchemy pour la table idps.error_events
"""
...
class ErrorEventRepository:
    ...
    def insert_events(self, data: List[Dict[str, Any]]) -> int:
        """Insère une liste d'événements d'erreur"""
        if not data:
            return 0
    ...
    def get_events(self, limit: int = 100, offset: int = 0) -> List[Dict[str, Any]]:
        """Récupère les événements d'erreur récents"""
        try:
            with self._get_session() as session:
                events = (
                    session.query(IDPSErrorEventModel)
                    .order_by(IDPSErrorEventModel.event_timestamp.desc())
                    .limit(limit)
                    .offset(offset)
                    .all()
                )
            ...
            error_msg = f"Erreur SQLAlchemy lors de la récupération des événements d'erreur: {e}"
            logger.error(error_msg)
